...
import { getHttpCodeErrorTitle } from '../utils/errorUtils';
...
export class AxiosProvider {
  ...
  private onAccessTokenFetched(accessToken: string) {
    this.subscribers = this.subscribers.filter(
      (callback: (accessToken: string) => void) => callback(accessToken),
    );
  }
  ...
  public setAccessToken() {
    ...
    auth
      && (this.axiosInstance.defaults.headers.common.Authorization = `Bearer ${auth.access_token}`);
  }
  ...
  private setResponseInterceptors() {
    this.axiosInstance.interceptors.response.use(
      ...
      async error => {
        ...
        if (
          errorStatusCode
          && Object.keys(HTTP_STATUS)?.includes(errorStatusCode?.toString())
        ) {
          apiError.errorLabel = errorMessage;
          apiError.errorTitle = getHttpCodeErrorTitle(errorCode);
          apiError.useCase = ApiErrorUseCase.SHOW_MESSAGE;
        }

        if (!error || !errorResponse) {
          apiError.errorLabel = 'GLOBAL.API_ERROR.NETWORK.DESCRIPTION';
          apiError.errorTitle = getHttpCodeErrorTitle(0);
          apiError.useCase = ApiErrorUseCase.SHOW_MESSAGE;
        }
        ...
        if (errorStatusCode === HTTP_STATUS[401].code) {
          ...
          if (auth) {
            if (
              ...
              && originalRequest.url.indexOf(ENDPOINT.REFRESH_TOKEN) > -1
            ) {
              this.isAlreadyFetchingAccessToken = false;
            ...
            if (!this.isAlreadyFetchingAccessToken) {
              ...
              try {
                ...
                localStorage.setItem(LocalStorage.ACCESS_TOKEN, JSON.stringify(authValue.data));
                this.setAccessToken();
